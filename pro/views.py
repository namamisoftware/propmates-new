...
def create_holiday_property(request):
    ...
    # ----- 2. File uploads -----
    images = request.FILES.getlist("media")            # multiple images
    ...
    # Add file fields to data if present
    if profile_photo:
    ...
    # NOTE: video_url normal field hota h so data se hi aa jayega
    # meta_title/meta_description/tags/featured/is_featured/sort_order also auto included
    ...
    if serializer.is_valid():
        ...
        # ----- 4. Save multiple gallery images -----
        for img in images:
...
def create_agriculture_property(request):  
    ...
    # FIX: Default payment_frequency if empty
    if not data.get("payment_frequency"):
    ...
    serializer = AgriculturePropertyCreateSerializer(data=data)

    if serializer.is_valid():
        property_obj = serializer.save()

        # 360 + Video
        if image_360 or video_file:
        ...
        return Response(AgriculturePropertySerializer(property_obj).data, status=201)
...
def create_pg_property(request):
    ...
    if serializer.is_valid():
        ...
        return Response(
            PGPropertySerializer(property_obj).data,  
            status=201
        )
...
def create_business_for_sale(request):
    ...
    # -----------------------------------------
    # ⭐ FIX: total_area ko number me convert karo
    try:
        data["total_area"] = float(data.get("total_area", 0))
    except:
        data["total_area"] = 0  
    ...
    if serializer.is_valid():
        ...
        return Response(
            BusinessForSaleSerializer(property_obj).data,
            status=201
        )
     # ------------------------------
...
class VerifyOTPAPIView(APIView):  
    def post(self, request):
        try:
            ...
            # OTP match → Create user from cached data
            ...
            if serializer.is_valid():
                ...
                custom_user = CustomUser.objects.create(
                    ...
                    password=make_password(user_data.get("password")),  # Secure
                )  
