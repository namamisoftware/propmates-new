...
def addCompany(request, pk=None):      
    company = RealEstateCompany.objects.get(pk=pk) if pk else None

    if request.method == "POST":

        # ----------- SAFE SINGLE VALUE FUNCTION ---------------
        def get_value(key):
        ...
        data["email"] = get_value("email")
        data["phone"] = get_value("phone")
        data["registration_number"] = get_value("registration_number")
        data["address"] = get_value("address")
        data["slug"] = get_value("slug")
        data["website"] = get_value("website")
        data["city"] = get_value("city")
        data["state"] = get_value("state")
        data["country"] = get_value("country")
        data["pincode"] = get_value("pincode")

        # Integer safely
        exp = get_value("experience_years")
        data["experience_years"] = int(exp) if exp else 0

        # Boolean safely
        verified = get_value("verified")
        data["verified"] = True if verified == "True" else False
        ...
        # ------------------ CREATE -------------------
            ...
            company = RealEstateCompany(**data)
            if image:
                company.image = image
            company.save()
    ...
    return render(request, "dashboard/add_company.html", {"row": company})
...
def verify_agent(request, agent_id):  
    ...
    user = CustomUser.objects.create_user(
        ...
        Country=agent.Pcountry,
        state=agent.Pstate,
        city=agent.Pcity,
        locality=agent.Plocality  
...
def add_property_wizard(request):  
    ...
    if step == "base1":
        if request.method == "POST":
            wizard_data.update({
                ...
                # New Fields   
                "bedrooms": int(request.POST.get("bedrooms")) if request.POST.get("bedrooms") not in ["", None] else None,
                "bathrooms": int(request.POST.get("bathrooms")) if request.POST.get("bathrooms") not in ["", None] else None,
  ...
  # -------------------------------------------------------
# ✅ STEP 5 — IMAGES & FINAL SAVE (UPDATED)
# -------------------------------------------------------
    if step == "images":
        if request.method == "POST":
            ...
            base = base_model.objects.create(
               ...
               bedrooms = int(wizard_data.get("bedrooms")) if wizard_data.get("bedrooms") not in ["", None] else None,
               bathrooms = int(wizard_data.get("bathrooms")) if wizard_data.get("bathrooms") not in ["", None] else None,
  
                balcony=wizard_data.get("balcony") == "on",
                view=wizard_data.get("view"),
    
                agent_id=wizard_data["agent"],
                property_type_id=wizard_data["property_type"],
            )
    
            base.amenities.set(wizard_data["amenities"])
    
            # -------------------------------
            # 2️⃣ Save subtype-specific model
            # -------------------------------
            commercial = {
                "office": Office,
                "shop": Shop,
            ...
            # -------------------------------
            # 3️⃣ Create MEDIA MODEL based on Category
            # -------------------------------
            if category == "commercial":
                MediaModel = CommercialPropertyImage
            else:
                MediaModel = ResidentialPropertyImage
    
            # -------------------------------
            # 4️⃣ Save ALL media entries
            # -------------------------------
            media_obj = MediaModel.objects.create(
                property=base,
            ...
            print("Media model used:", media_obj)   
            # -------------------------------
            # 5️⃣ Clear Session
            # -------------------------------
...
def property_list(request): 
    ...
    # ⭐⭐ NEW: Agent Filtering ⭐⭐
    # ----------------------------------------------------
    locality = getattr(prop, "locality", "").strip()
    city = getattr(prop, "city", "").strip()

    # 1) Locality match first
    agents_qs = CustomUser.objects.filter(
        locality__iexact=locality,
         
    )  

    # 2) If locality agents not found → city agents
    ...
    # Attach filtered agents to this property object
...
def agriculture_create(request):
    if request.method == "POST":
        ...
        user_id = data.get("Client")
        company_obj = RealEstateCompany.objects.get(id=company_id) if company_id else None
        user_obj = CustomUser.objects.get(id=user_id) if company_id else None
    ...
    company =  RealEstateCompany.objects.all()
    User = regesteruser.objects.all()  
    return render(request, "dashboard/AgriculturePcreate.html", {"amenities": amenities, "property_types":property_types, "companys" : company, "Users": User})  
...
def agriculture_update(request, pk):
    ...
    company = RealEstateCompany.objects.all()
    User = regesteruser.objects.all()
    return render(request, "dashboard/agriculture/edit.html", {"property": property_obj,
    ...
    if request.method == "POST":
        try:
            ...
            single_images = {
                ...
                # ---------- Single Images ----------
            for field, file in single_images.items():
                if file:
...
def business_create(request):
    ...
    company =  RealEstateCompany.objects.all()
    User = regesteruser.objects.all()
    return render(request, "dashboard/Business_Sale_create.html", {"amenities": amenities, "property_types": property_types,
                                                                    "companys": company,
                                                                   "Users" : User })   
